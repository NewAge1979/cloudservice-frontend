...
export default defineComponent({
  name: "FilesHome",
  components: {EButton, ECheckbox, EUpload},
  setup() {
    ...
    const uploadNewFile = (file: any) => {
      store.dispatch('addNotification',{
        ...
        uploadFile(file, file.name)
            ...
            .then(() => {
              setTimeout(() => {notification.hidden = true}, 5000);
            });
      })
    };
        ...
        deleteFile(file.filename)
            .then((res: any) => {
              setTimeout(() => {
                ...
                notification.status = 'success';
                notification.text = `Файл "${file.filename}" успешно удален`;
              }, 2000);
            })
            .catch((res: any) => {
              setTimeout(() => {
    ...
    const selectedDownload = () => {
      ...
      selectedFiles.forEach((f: any) => fileDownload(f));
    };

    const selectedDelete = () => {
      const selectedFiles = files.filter((f: any) => f.selected);

      selectedFiles.forEach((f: any) => fileDelete(f));
    };

    return {
      sortColumn, sortColumnReverse, files, allFilesSelection, onFileSelect, uploadNewFile, preparedFiles,
      getExtColor, getFormattedDate, getFormattedSize, getFileNameWithoutExt, isSeveralFilesSelected,
      changeSort, selectAllFiles, fileChangeName, fileDownload, fileDelete, selectedDownload, selectedDelete
    };
  }
});
</script>
...
<template>
  <main class="home">
    <div class="container">
      <h1 class="home__title">Ваши файлы</h1>
      <e-upload class="home__upload" @update="uploadNewFile" />
      <table class="home__files files">
        <thead>
          <tr>
            <th class="files__header files__header-select" :class="{'files__header-select--show': isSeveralFilesSelected}">
              <e-checkbox v-model="allFilesSelection" :terminated="isSeveralFilesSelected" @change="selectAllFiles" />
            </th>
            <th class="files__header" @click="changeSort('filename')">
              Название
            ...
            <th class="files__header" @click="changeSort('editedAt')">
              Дата изменения
              <img v-if="sortColumn === 'editedAt' && sortColumnReverse" src="@/assets/icons/down-arrow.svg">
              <img v-if="sortColumn === 'editedAt' && !sortColumnReverse" src="@/assets/icons/up-arrow.svg">
            </th>
            <th class="files__header" @click="changeSort('size')">
              Размер
            ...
            <th class="files__header"></th>
          </tr>
          <tr>
            <th></th>
            <th v-if="isSeveralFilesSelected" class="files__global-actions" colspan="4">
              <e-button type="plain" @click="selectedDownload">Скачать выбранные</e-button>
              <e-button type="plain" @click="selectedDelete">Удалить выбранные</e-button>
            </th>
          </tr>
        </thead>
        <tbody>
          <tr
              v-for="(file, fileIndex) in preparedFiles"
              :key="fileIndex"
              :class="{
          ...
          >
            <td class="files__cell files__cell--select"><e-checkbox v-model="file.selected" @change="onFileSelect" /></td>
            <td :title="file.filename" class="files__cell" @click="file.selected = !file.selected">
              <div
                :class="[`files__extension-icon--length-${file.ext.length}`]"
                :style="{borderColor: getExtColor(file.ext), color: getExtColor(file.ext)}"
            ...
            <td class="files__cell" @click="file.selected = !file.selected">
              <span>{{getFormattedDate(file.editedAt)}}</span>
            </td>
            <td class="files__cell" @click="file.selected = !file.selected">
              <span>{{getFormattedSize(file.size)}}</span>
            </td>
            <td class="files__cell files__cell--actions">
              <e-button title="Изменить" type="text" @click="fileChangeName(file)"><img src="@/assets/icons/edit.svg"></e-button>
              <e-button title="Скачать" type="text" @click="fileDownload(file)"><img src="@/assets/icons/download.svg"></e-button>
              <e-button title="Удалить" type="text" @click="fileDelete(file)"><img src="@/assets/icons/delete.svg"></e-button>
            </td>
          </tr>
        </tbody>
      </table>
